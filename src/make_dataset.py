import os
import numpy as np
from config import PATH_TRAIN_OUTPUT, PATH_TRAIN_INPUT, IM_WIDTH, IM_HEIGHT
from random import shuffle
import tensorflow 
from tensorflow.keras.preprocessing.image import img_to_array, load_img
from skimage.transform import resize

# print("PATH_TRAIN_OUTPUT:", PATH_TRAIN_OUTPUT)
if not os.path.exists(PATH_TRAIN_OUTPUT):
    raise FileNotFoundError(f"Directory {PATH_TRAIN_OUTPUT} does not exist.")
files = os.listdir(PATH_TRAIN_OUTPUT)
if not files:
    raise FileNotFoundError(f"No files found in {PATH_TRAIN_OUTPUT}.")
ids = [f for f in files if os.path.isfile(os.path.join(PATH_TRAIN_OUTPUT, f))]
print(len(ids))
